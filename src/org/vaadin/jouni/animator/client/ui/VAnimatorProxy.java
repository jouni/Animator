...
public class VAnimatorProxy extends SimplePanel implements Paintable {
        ...
        protected void onUpdate(double progress) {
            if (dir > 0) {
                ...
                if (BrowserInfo.get().isIE8()) {
                    target.getElement().getStyle().setProperty(
                            "filter",
    ...
    private Timer queue = new Timer() {
        @Override
        public void run() {
            client.updateVariable(paintableId, "anim", animEvents, true);
            animEvents.clear();
        }
    };
    ...
    protected class RollAnimation extends Animation {
        ...
        protected void onComplete() {
            super.onComplete();
            if (origOverflow != null && origOverflow != "") {
                target.getElement().getStyle().setProperty("overflow",
                        origOverflow);
            } else if (dir == 1 || dir == 3) {
    ...
    protected class SizeAnimation extends Animation {
        ...
        protected void onStart() {
            ...
            if (animatePosition) {
                if (!isWindow) {
                    target.getElement().getStyle().setPosition(
                            Position.ABSOLUTE);
                }
                ...
                if (top != null && top != "") {
                    startY = Integer.parseInt(top
                            .substring(0, top.length() - 2));
                } else {
                    startY = 0;
                }
        ...
        protected void onUpdate(double progress) {
            ...
            if (isWindow && animatePosition) {
                ...
                target.getElement().getStyle().setLeft(
                        (int) (startX + progress * (endX - startX)), Unit.PX);
                target.getElement().getStyle().setTop(
                        (int) (startY + progress * (endY - startY)), Unit.PX);
            }
